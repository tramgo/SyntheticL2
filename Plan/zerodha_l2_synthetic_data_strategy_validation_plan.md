...
The current completed run processed all 2,259,039 Phase 9 Tier C 5-minute feature rows, applied Phase 11 proxy signals, and simulated 9 supported/proxy strategies across 3 execution profiles: `zero_latency_spread_only_control`, `retail_marketable_default` and `stressed_retail`. It produced 27 strategy/profile summary rows and a 249,993-row deterministic stratified sampled trade ledger covering all 27 strategy/profile combinations. The sampled trade ledger now carries proxy `volatility_bucket` and `liquidity_bucket` fields derived from Phase 9 local volatility, event intensity and spread, plus a deterministic `seed`/`simulation_seed` lineage assigned from the Phase 13 seed plan by quarter profile and scenario day. The current sample exposes all 30 Phase 13 registered seeds, 10 per quarter profile. Across all strategy/profile combinations the run simulated 10,350,654 marketable proxy trades.

The simulator applies event-latency shifts, stale/disconnect cancellation, half-spread marketable execution cost, fixed slippage ticks, internal impact bps and a Zerodha-sourced normalized equity-intraday NSE round-trip charge estimate. The current `cost_schedule.csv` records brokerage, STT, NSE transaction charges, SEBI charges, stamp duty and GST from Zerodha's published charge schedule as a 10.60688 bps normalized proxy for retail/stressed profiles. The companion `charge_component_catalog.csv` and `representative_charge_scenarios.csv` now add a rupee order-formula ledger for Zerodha equity-intraday NSE charges, including the Rs. 20/order brokerage cap, sell-side STT with nearest-rupee rounding, transaction charges, SEBI charges, buy-side stamp duty and GST. The representative scenarios cover Rs. 10,000 through Rs. 500,000 buy/sell notionals and show effective round-trip costs declining from 11.12812 bps to 4.49212 bps on buy notional as the brokerage cap binds. This is better than a pure bps placeholder, but it is still not acceptance-grade because actual broker fills, broker contract-note reconciliation, DP/depository charges where applicable, and full tick/order lifecycle are not verified.
...
The current completed run audits 10 required reproducibility fields across 25 phase/workspace/dashboard manifests, including Phase 2-8 upstream calibration/generation manifests, the Phase 1 event-reconstruction manifest, Phase 11 strategy-module manifest, Phase 12 execution manifest, Phase 12 event-backtest manifest, Phase 15 acceptance-gates manifest, Phase 16 metrics manifest, Phase 17 work-package manifest and DuckDB workspace manifest, producing 250 field checks. Current coverage is improved but not sufficient for full exact regeneration: 15 artifacts are exact-regeneration-ready at the source-manifest level, 9 artifacts still have missing fields, and 1 artifact group still has a missing/unreadable manifest. The exact-ready source manifests now include `duckdb`, `phase1_event_reconstruction`, `phase2`, `phase3`, `phase4`, `phase5`, `phase6`, `phase7`, `phase8`, `phase11_strategy_modules`, `phase12`, `phase12_event_backtest`, `phase15`, `phase16` and `phase17`; earlier remaining phases generally still record `generated_utc` and selected input paths but do not yet consistently record generator version, configuration hash, seed, calibration dataset ID, ticker metadata version, regime calendar version, scenario IDs, cost model version and latency model version.

The remediation layer now emits a normalized reproducibility manifest template and 250 field-level remediation rows: 80 `add_field_in_generator` actions, 10 `normalize_alias_to_exact_field` actions, 10 `recover_or_rerun_manifest` actions and 150 `complete_exact` rows. This converts the audit into a generator-by-generator implementation checklist without pretending the remaining historical artifacts are already exactly reproducible.

The normalized manifest overlay now creates exact-field manifest overlays for all 25 audited artifacts without rewriting their historical source manifests. The overlay has 25 exact-field-ready artifacts and 250 normalized field rows: 160 values came from exact/alias fields already present in source manifests and 90 values were supplied by the normalizer using current git/config/source defaults. This is a reproducibility bridge for current audit artifacts, not proof that every original phase generator already emits normalized manifests.

Important Phase 19 caveat: this phase is now an audit plus remediation/normalization layer with native generator retrofits for 15 audited artifacts, not a completed whole-workspace generator retrofit. The next implementation step is to adopt the normalized manifest schema inside the remaining phase runners and rerun/backfill artifacts with exact regeneration metadata rather than relying on overlay defaults.
