...
The remediation layer now emits a normalized reproducibility manifest template and 250 field-level remediation rows. All 250 rows are `complete_exact`, confirming that the audited source manifests now expose the exact required fields without generator-field, alias-normalization or recover/rerun gaps.
