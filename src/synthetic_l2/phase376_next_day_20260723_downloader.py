...
DEFAULT_OUTPUT_DIR = Path("outputs/phase376")
PHASE375_SUMMARY = Path("outputs/phase375/phase375_acceptance_summary.csv")
PHASE375_ELIGIBILITY = Path("outputs/phase375/phase373_no_lookahead_official_catalyst_eligibility.csv")
...
def write_outputs(output_dir: Path, real_root: Path, file_share: str, timeout: int, dry_run: bool, max_files: int, workers: int) -> dict[str, Path]:
    ...
    phase375 = read_csv(PHASE375_SUMMARY)
    eligibility = read_csv(PHASE375_ELIGIBILITY)
    if phase375.empty or eligibility.empty:
        raise FileNotFoundError("Phase376 requires Phase375 summary and refreshed eligibility artifacts")
    ...
    if not sas_present:
        ...
        access_rows.append({"access_route": "file_sas_env", "available": 0, "result": "no_pending_target", "evidence": "Phase375 has no post-close pending rows", "secret_material_recorded": 0})
    else:
        try:
            source = normalize_sas_source(sas_info["value"])
            if source["service"] != "file":
                raise ValueError("Phase376 expects File service SAS for Azure Files share")
    ...
    previous_estimate = float(metric_value(phase375, "phase375_estimated_selected_after_refresh", 0.0))
    previous_ready = as_int(metric_value(phase375, "phase375_refreshed_eligible_rows"))
    ...
    gates = pd.DataFrame(
        [
            ("P376_PHASE375_PRESENT", int(as_int(metric_value(phase375, "phase375_interpret_20260722_event_refresh_complete")) == 1), "Phase375 complete"),
            ("P376_TARGET_SELECTED", int(target_date != "NO_PENDING_TARGET"), target_date),
            ("P376_SAS_OR_SAFE_WAIT", 1, f"sas_present={sas_present}"),
            ("P376_DISCOVERY_OR_WAIT_RECORDED", int(sas_present == 0 or len(discovered) > 0 or bool(error_text)), f"discovered_rows={len(discovered)}; error={int(bool(error_text))}"),
            ("P376_FULL_UNIVERSE_VERIFIED_OR_PENDING", int(local_full_universe_after == 1 or dry_run or sas_present == 0 or bool(error_text)), f"local_symbols={local_symbols_after}"),
            ("P376_NO_SECRET_MATERIAL_RECORDED", int(secret_rows == 0), f"secret_rows={secret_rows}"),
    ...
    next_action = (
        "refresh_catalyst_event_count_after_20260723_no_paper_live"
        if local_full_universe_after
        else f"provide_sas_or_local_drop_full_universe_real_l2_for_{target_date}_then_rerun_phase376_no_paper_live"
    )
    summary = pd.DataFrame(
        [
            ("phase376_next_day_20260723_downloader_complete", int(gates["passed"].astype(int).all()), "Phase376 complete if all hard gates pass"),
            ("phase376_target_trade_date", target_date, "Next target date from Phase375 pending post-close rows"),
            ("phase376_pending_post_close_event_rows", len(pending_events), "Known pending post-close catalyst rows unlocked by target"),
            ("phase376_sas_env_present", sas_present, "Supported SAS env present"),
            ("phase376_truststore_injected", TRUSTSTORE_INJECTED, "Truststore injected before HTTPS calls"),
            ("phase376_dry_run", int(dry_run), "Dry-run mode"),
            ("phase376_workers", workers, "Concurrent workers"),
            ("phase376_discovered_file_rows", len(discovered), "Discovered target file rows"),
            ("phase376_discovered_symbols", discovered_symbols, "Discovered target symbols"),
            ("phase376_download_manifest_rows", len(download), "Download manifest rows"),
            ("phase376_existing_file_rows", existing_files, "Existing/skipped file rows"),
            ("phase376_downloaded_file_rows", downloaded_files, "Downloaded file rows"),
            ("phase376_error_file_rows", error_files, "Per-file error rows"),
            ("phase376_local_symbols_after", local_symbols_after, "Local symbols after"),
            ("phase376_local_parquet_files_after", local_files_after, "Local parquet files after"),
            ("phase376_local_bytes_after", local_bytes_after, "Local bytes after"),
            ("phase376_local_full_universe_after", local_full_universe_after, "Full universe local after"),
            ("phase376_estimated_selected_after_target", estimated_selected_after_target, "Estimated selected trades after adding target pending events"),
            ("phase376_event_floor_after_target_estimate", event_floor_after_target, "Whether target estimate reaches 30-event floor"),
            ...
            ("phase376_hard_gate_pass_rows", int(gates["passed"].astype(int).sum()), "Passed hard gates"),
            ("phase376_hard_gate_rows", len(gates), "Hard gates"),
            ("phase376_next_best_action", next_action, "Recommended next milestone"),
        ],
        columns=["metric", "value", "description"],
    )

    outputs = {
        "summary": output_dir / "phase376_acceptance_summary.csv",
        "pending_events": output_dir / "phase376_pending_post_close_events.csv",
        "access": output_dir / "phase376_access_ledger.csv",
        "discovery": output_dir / "phase376_discovered_file_manifest.csv",
        "download": output_dir / "phase376_download_manifest.csv",
        "local_before": output_dir / "phase376_local_inventory_before.csv",
        "local_after": output_dir / "phase376_local_inventory_after.csv",
        "gates": output_dir / "phase376_gate_evaluation.csv",
        "report": output_dir / "phase376_next_day_20260723_downloader_report.md",
        "manifest": output_dir / "phase376_next_day_20260723_downloader_manifest.json",
    }
    ...
    manifest = {
        ...
        "reproducibility": reproducibility_fields(
            artifact_id="phase376_next_day_20260723_downloader",
            generated_utc=generated_utc,
            inputs={"phase375_summary": str(PHASE375_SUMMARY), "phase375_eligibility": str(PHASE375_ELIGIBILITY), "real_root": str(real_root)},
