...
def build_capacity_policy_catalog() -> pd.DataFrame:
    return pd.DataFrame(
        [
            {
                "capacity_policy_id": "P481_BASELINE_MAX3_CONCURRENT",
                "policy_role": "baseline",
                "selection_rule": "reproduce Phase387 apply_capacity: sort by decision_ms, canonical_work_order_id; keep if active exits < 3",
            ...
            {
                "capacity_policy_id": "P481_MAX5_CONCURRENT",
                "policy_role": "looser_capacity",
                "selection_rule": "same active-exit overlap logic as baseline but max concurrent positions = 5",
                "max_concurrent_positions": 5,
                "per_symbol_date_cap": "",
                "per_trade_date_cap": "",
                "all_ready_events": 0,
                "acceptance_role": "sensitivity",
            },
            {
                "capacity_policy_id": "P481_ONE_PER_SYMBOL_DATE",
                "policy_role": "breadth_first",
                "selection_rule": "after readiness and signal filters, keep earliest candidate per scenario, diagnostic_trade_date, symbol",
            ...
            },
            {
                ...
                "policy_role": "date_balanced",
                "selection_rule": "after readiness and signal filters, keep earliest two candidates per scenario and diagnostic_trade_date",
                "max_concurrent_positions": "",
                "per_symbol_date_cap": "",
...
def build_gates(phase480: pd.DataFrame, work: pd.DataFrame, trades: pd.DataFrame, catalog: pd.DataFrame, contract: pd.DataFrame) -> pd.DataFrame:
    ...
    rows = [
        ...
        ("P481_CAPACITY_POLICY_GRID_FROZEN", len(catalog) == 5, len(catalog), 5),
