...
def _densify_chunk(
    ...
    repeated["callback_received_utc_ms"] = (
        repeated["callback_received_utc_ms"].astype("int64") + repeated["dense_subtick_id"].astype("int64")
    )
    repeated["callback_received_monotonic_ns"] = repeated["callback_received_utc_ms"].astype("int64") * 1_000_000
    repeated["exchange_timestamp_ms"] = (
        repeated["exchange_timestamp_ms"].astype("int64") + repeated["dense_subtick_id"].astype("int64")
    )
    ...
    micro_step = (
        repeated["sell_1_price"].astype(float) - repeated["buy_1_price"].astype(float)
    ).abs().clip(lower=0.01) * 0.08
